...
Baseline gene body methylation was positively correlated with expression across all genes (Spearman $\rho = 0.407$, $p < 10^{-80}$, $n = 16{,}958$), with the strongest correlation in exonic regions ($\rho = 0.407$) and the weakest in promoters ($\rho = -0.015$) (Fig.~\ref{fig:metagene}G,~H).
...
\begin{figure}[ht]
    ...
    \caption{\textbf{Metagene methylation profiles and expression-dependent gene body methylation.}
    ...
    (\textbf{G}) Methylation--expression Spearman correlation by genomic region.
    (\textbf{H}) Mean gene body methylation by expression decile (D1=lowest, D10=highest).
...
\begin{figure}[ht]
    ...
    \begin{subfigure}[b]{0.90\textwidth}
        \centering
        \includegraphics[width=\textwidth]{fig6_dmp_go_all_dotplot.pdf}
        \caption{}
    \end{subfigure}

    \caption{\textbf{Tail amputation reshapes the \textit{D.\ laeve} methylation landscape.}
    (\textbf{A}) Volcano plot of differentially methylated positions (DMPs; 17,729 total: 8,285 hyper, 9,444 hypo).
    (\textbf{B}) Volcano plot of differentially methylated regions (DMRs; 1,462 total: 690 hyper, 772 hypo).
    (\textbf{C}) Genes ranked by DMP burden (number of significant DMPs per gene).
    (\textbf{D}) Genes ranked by DMR burden.
    (\textbf{E}) Gene Ontology enrichment analysis of all DMP-associated genes.}
...
\begin{figure}[ht]
    ...
    \caption{\textbf{Differential methylation is decoupled from transcriptional changes during regeneration.}
    ...
    (\textbf{E}) Dose--response: DMP burden bins vs.\ differential expression rate.
    (\textbf{F}) DE rate among DMP-carrying genes by genomic region.}
